...
wb = Workbook() # 创建文件对象
# road = r"E:\Moredian\tools\stability\20230821_174352\com.moredian.mdservice\DumpDevStateService"
ws = wb.active
q = w = a = s = d = f = g = 2
# ws["A1"] = "系统时间"
# ws["B1"] = "argus进程pid"
...
def quality_D2C_cabinet(road, eachFile=eachFile):
    System_time_list = []
    CPU_radio_list = []
    apk_cpu_radio_list = []
    Free_RAM_list = []
    ...
    for eachDir in pathDir:
        try:
            ...
            for lines in fopen.readlines():
                ...
                if "u0_a27" in str(lines) and "com.moredian.entrance.guard" in str(lines):    # D2PLUS
                    apk_cpu_radio = float(lines[lines.find("com.moredian.entrance.guard") - 21 :lines.find("com.moredian.entrance.guard")-17])
                    # Write(apk_cpu_radio_list_row, apk_cpu_radio)
                    apk_cpu_radio_list.append(int(float(apk_cpu_radio))/6)
                    s += 1
    ...
    Free_RAM = "系统剩余内存：" + str(round(sum(Free_RAM_list) / len(Free_RAM_list), 2)) + "kb(" + str(min(Free_RAM_list)) + "kb - " + str(max(Free_RAM_list)) + "kb)"
    apk_RAM = "apk占用内存：" + str(round(sum(apk_RAM_list)/len(apk_RAM_list),2)) + "kb(" + str(min(apk_RAM_list)) +"kb - " + str(max(apk_RAM_list)) + "kb)"
    Temp = "设备温度：" + str(round(sum(Temp_list)/len(Temp_list),2)) + "℃(" + str(min(Temp_list)) +"℃ - " + str(max(Temp_list)) + "℃)"
    tuple = (CPU_radio,apk_cpu_radio,Free_RAM,apk_RAM,Temp)
    return tuple
...
def finddata():
    ...
    try:
        if filepath.get() == "":
            lb.insert(END, "文件路径不能为空！")
        elif product.get() == "D2C储物柜":
            tuple = quality_D2C_cabinet(path)
            for i in tuple:
                lb.insert(END,str(i))
    except Exception as e:
        lb.insert(END, e)
...
DealFile_Path = Radiobutton(root,text="D2C储物柜",variable = product,value = "D2C储物柜",borderwidth=1,width=15,anchor=W).place(x=150, y=50, anchor="se")
