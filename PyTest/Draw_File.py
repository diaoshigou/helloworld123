...
def D2P(path,files,CPU_Date,Free_Date,APK_CPU_Date,APK_MEM_Date,CPU_TEMP_Date,Argus_PID_Date,CPU_radio_list,Free_RAM_list,apk_cpu_radio_list,apk_RAM_list,Temp_list,Pid_list):
    CPU_current_type = CPU_item_type = None
    MEM_current_type = MEM_item_type = None
    APK_CPU_current_type = APK_CPU_item_type = None
    APK_MEM_current_type = APK_MEM_item_type = None
    CPU_TEMP_current_type = CPU_TEMP_item_type = None
    Argus_PID_current_type = Argus_PID_item_type = None
    ...
    for file in files:
        ...
        for lines in fopen.readlines():
            ...
            if "u0_a" in str(lines) and "com.moredian.entrance.guard" in str(lines) and "S" in lines and "channel" not in lines:  # APK_CPU
            ...
            if "u0_a" in str(lines) and "com.moredian.entrance.guard" in str(lines) and "R" in lines and "channel" not in lines:  # APK_CPU
                APK_CPU_item_type = "data"
                if APK_CPU_current_type == "time":
                    APK_CPU_current_type = APK_CPU_item_type
            ...
            if "moredianMem" in str(lines):                                                                             #APK_MEM
                ...
                if APK_MEM_current_type == "time":
                    ...
                    apk_RAM = int(str(lines[lines.find(":") + 1:lines.find(",")])+str(lines[lines.find(",") + 1:lines.find("K")]))  # APK_MEM
...
# D2P(path,files,CPU_Date=[],Free_Date=[],APK_CPU_Date=[],APK_MEM_Date=[],CPU_TEMP_Date=[],Argus_PID_Date=[],CPU_radio_list=[],Free_RAM_list=[],apk_cpu_radio_list=[],apk_RAM_list=[],Temp_list=[],Pid_list=[])
# D3(path,files,CPU_Date=[],Free_Date=[],APK_CPU_Date=[],APK_MEM_Date=[],CPU_TEMP_Date=[],Argus_PID_Date=[],CPU_radio_list=[],Free_RAM_list=[],apk_cpu_radio_list=[],apk_RAM_list=[],Temp_list=[],Pid_list=[])
# D2_D2A(path,files,CPU_Date=[],Free_Date=[],APK_CPU_Date=[],APK_MEM_Date=[],CPU_TEMP_Date=[],Argus_PID_Date=[],CPU_radio_list=[],Free_RAM_list=[],apk_cpu_radio_list=[],apk_RAM_list=[],Temp_list=[],Pid_list=[])
...
Var.set(1)
filepath = StringVar()
# filepath.set(r"E:\Moredian\tools\stability\20240419_102922\com.moredian.mdservice\DumpDevStateService")



Data_FilePath = tkinter.Label(root,text="文件路径：",borderwidth=1,width=10,anchor=NE).place(x=20,y=80)
Entry_FilePath = Entry(root, borderwidth=1,textvariable=filepath,width=90).place(x=100,y=80)
...
comand = Button(root, text="获取", command=find_data,width=10, height=2,borderwidth=2).place(x=20,y=120)
